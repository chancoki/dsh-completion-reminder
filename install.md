...
> 钉钉与企业微信机器人接口不允许浏览器直连（CORS），v1.7.0 起已移除；
> 需要的话请走 Server酱或自建通用 Webhook 转发。
...
| 旧配置选的钉钉/企微升级后不见了 | v1.7.0 移除了不可直连的渠道 | 自动回落到浏览器通知，重新在单选组里选择即可 |
| 飞书报 19001 | Webhook token 错误 | 重新复制机器人 Webhook 地址 |
| 飞书提示网络失败 | Webhook 地址错或公司网络拦截 | 核对地址；企业内网可能需要代理出网 |
