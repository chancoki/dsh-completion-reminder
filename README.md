...
| **钉钉机器人** | 钉钉群 | Webhook 地址 / 加签密钥（可选）/ 本地转发地址 |
| **飞书机器人** | 飞书群 | Webhook 地址 / 签名密钥（可选）— 直连可用 |
| **企业微信机器人** | 企业微信群 | Webhook 地址 / 本地转发地址 |
...
> **国内渠道连通性说明**（2026-08 实测三家线上接口得出）：
> - **飞书**：接口自带完整 CORS 头，浏览器**直连可用**，零配置；
> - **钉钉 / 企业微信**：接口不返回 CORS 头，且钉钉强制要求
>   `application/json`（会触发被拒的预检，text/plain 又被 43004 拒收）——
>   浏览器物理上无法直连。插件附带零依赖转发小服务 `relay.mjs`，
>   一条命令启动后把「本地转发地址」填为 `http://127.0.0.1:8765` 即可；
>   签名仍在插件本地计算，密钥不出机器。钉钉机器人若开了「自定义关键词」
>   过滤，请把关键词设为 `DSH`。
